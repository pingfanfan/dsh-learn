...
本地状态账本中保留了一些人工批准闸门启用前的知乎成功回执。这些 URL 只用于历史追溯，不构成对任何新任务的授权，也不代表本轮发布过知乎内容。

当前知乎渠道是 `DRAFT_ONLY` 且 `approvalRequired: true`：任何新的知乎 `PublishJob` 都必须先有主理人针对该任务的明确 `userApproval`，没有批准时只能保存草稿，不能进入知乎创作中心，也不能写入成功回执。当前系统没有使用或保存 DeepSeek API Key；外部源不可达时只能记录扫描降级或使用已由连接器核对的公开 revision attestation，不能把网络失败解释成上游没有变化。
...
- [x] 多资产机会会在所有已登记资产验收前保留租约；首个资产完成不会释放后续资产的写回权限，已有草稿资产也可通过 `asset-revise` 安全恢复验收。
...
- [x] 已记录浏览器渠道中继手册，并完成五条微博内容及账本中五篇历史知乎文章的真实回执与去重核对；已有阅读指标继续回流，未确认 URL 时仍停留在 `OUTBOX`。新的知乎任务仍受当前人工批准闸门约束。
...
总控队列已完成二十三项高分机会的首个规范资产：官方变更事实站、Discussions 最小复现工具包、官方 Discussions 社区入口卡、Discussions 新问题分流卡、国产模型/网关矩阵、第三方插件迁移诊所、扩展点中文能力地图、安全边界实测手册、Cordis 无 Key mini-lab、Python SDK 安全自动化配方、DSH 无 Key 新手入口、DSH plugin 术语 FAQ、官方会话内容搜索 opt-in 教程、旧版 Discussions revision 复核卡、#535–#537 复核事实卡、#539–#549 复核事实卡、#550–#552 复核事实卡、#553–#559 复核事实卡、#560–#564 新手排障与网关兼容边界卡、#565–#591 新插件/工具调度报告与 Windows 启动排障卡、DSH 生态插件 npm rc.6 迁移事实卡，以及 dsh-plugin-check 当前 rc.6 安装说明。其中 FAQ 已在 GitHub、微博和知乎获得真实回执；Cordis mini-lab 已在 GitHub、微博和知乎获得真实回执；Python SDK 安全配方已在 GitHub、微博和知乎获得真实回执；中文 Discussions 模板、社区入口卡、Discussions 新问题分流卡、插件迁移诊所、安全边界手册、provider 矩阵、扩展点能力地图和 DSH 无 Key 新手入口已在 GitHub 获得真实回执，其中社区入口卡已获得微博回执（`https://weibo.com/7621278794/RdhRIu2qL`），新问题分流卡微博稿已过门禁但暂未发布，知乎未发布。最新 #565–#591 复核卡已获得 GitHub 文件回执（`https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-discussions-revision-20260814-591.md`）；其当前证据基线为 7 页、700 条公开讨论、#12–#720。新手入口也已获得知乎文章回执。会话内容搜索教程已获得 GitHub 回执（`https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-session-content-search-opt-in-47f9438.md`）、微博回执（`https://weibo.com/7621278794/RdhnpeP8j`）和知乎文章回执（`https://zhuanlan.zhihu.com/p/2071432880262329422`）。dsh-plugin-check 旧版 rc.6 复核卡已退休，仅保留 8b3ea76 历史记录；当前安装说明与 npm rc.6 迁移卡已经分别获得 GitHub 文件回执（`https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-plugin-check-rc6-current-install-notes-20260814.md`、`https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-ecosystem-npm-rc6-migration-20260814.md`），没有安装第三方插件，知乎仍需主理人明确同意。旧版 Discussions revision 事实卡保留为历史版本，分页复核卡已随 #559、#564、#591 和 #614 新卡滚动更新；公众号不可用。旧版扩展点地图和旧版新手 GitHub outbox 的未知远端状态已通过渠道核验后安全归档，并按当前 revision 发布了清理后的公开版本。除已记录的真实回执外，本地 `MOCK`、Agent outbox 和 `DRAFT_ONLY` 都只证明工作流或待办已生成，不属于公开发布。
- [x] DSH 无 Key 新手入口已完成终稿润色、rc.6 固定证据、GitHub/微博真实回执和公众号渠道稿；账本中若有该主题的早期知乎历史回执，只作历史记录，当前新增知乎发布仍需主理人明确批准。模型、provider、Web UI 与第三方插件仍明确未覆盖。
...
- [x] #553–#559 维护复核已完成：原始观察仍固定在 #553–#559，当前来源基线更新为 7 页/700 条/#720；正文只更新维护基线并保留第三方 fork、gateway-presets 和真实网关未动态验证的边界，已通过润色与完整检查，GitHub 更新回执为 `https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-discussions-revision-20260814-559.md`，没有调用模型 API，知乎未发布。
- [x] #539–#549 维护复核已完成：原始观察仍固定在 #539–#549，当前来源基线更新为 7 页/700 条/#720；正文按 Web/host 边界、社区 bundle 结构和新手入口重写，已通过 polish-article 与今日头条终稿总检，社区插件没有下载、安装或启动，GitHub 更新回执为 `https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-discussions-revision-20260814-549.md`，没有调用模型 API，知乎未发布。
- [x] #565–#591 维护复核已完成：原始观察仍固定在 #565–#591，当前来源基线更新为 7 页/700 条/#720；正文按社区插件边界、Windows 端口排障、工具调度报告和完全新手的无 Key 入口重写，已通过 polish-article 与今日头条终稿总检，GitHub 更新回执为 `https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/dsh-discussions-revision-20260814-591.md`，没有下载、安装或运行第三方插件，没有调用模型 API，知乎未发布。
- [x] #37、#38、#40 新问题分流卡维护复核已完成：原始观察仍固定在 #37、#38、#40，当前来源基线更新为 7 页/700 条/#720；正文按浏览器窗口、依赖加载、Session 归档和完全新手证据回执重写，补充固定 commit 的 ui-workspace README 已知限制，已通过 polish-article 与今日头条终稿总检，GitHub 更新回执为 `https://github.com/pingfanfan/dsh-learn/blob/main/content/channels/github/discussion-triage-41.md`，没有在 Windows/Firefox 上动态复现，没有调用模型 API，知乎未发布。
