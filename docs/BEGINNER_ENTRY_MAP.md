...
- 新手环境检查脚本的静态验证：`pnpm validate:beginner-doctor` 已通过；追加 `node scripts/beginner-doctor.mjs --report` 可输出不含凭据、路径已隐藏的诊断回执（含 `SCREENSHOTS=PASS`、`SCREENSHOT_COUNT=11/11` 和 `KEY_STATUS=not_read`），不联网、不上传；缺少任一张新手图时，体检会在进入启动步骤前明确报错。
