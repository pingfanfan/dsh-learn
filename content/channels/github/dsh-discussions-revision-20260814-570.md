...
这张卡的原始观察发生在 `#565–#570` 出现时，官方 Discussions 当时是 6 页、556 条，最后编号为 `#570`，随后列表扩展到 6 页、600 条、`#614`，现在已经复核到第 11 页，最后编号为 `#1068`。本文仍专门复盘 `#565–#570` 这 6 条历史信号，列表变化只用于维护来源，不把后续讨论混进历史结论，也不能当成 DSH 发布了新版本。官方代码基线仍按 commit `47f9438` 记录，npm 上新手路径使用的仍是 `@deepseek-ai/dsh@0.1.0-rc.6`。
